...
//    cout << endl << "------- START OF NUMBER OF PORTIONS =-" << endl;

    int portion_count = 0; // step1 initialize portion count to 0

    for (list<stack<MealPortion*>*>::iterator i = stacks.begin(); i != stacks.end(); ++i) { // step2 use list iterator to iterate through list elements
        // iterate through stack elements
        stack<MealPortion*> cur_stack = **i;
        int cur_stack_size = cur_stack.size();
        portion_count += cur_stack_size; // step2.1 add current stack size to portion count
    }
//    cout << "------- END OF NUMBER OF PORTIONS =-" << endl << endl;

    return portion_count; // step3 return final portion count
}
...
//    cout << endl << "------- START OF ADDING MEAL =-" << endl;

    MealPortion* mp = new MealPortion(n_name, n_expiry);  // step1 create a new meal portion object on the heap

    stack<MealPortion*>* stack_pointer = stacks.back();  // step2 grab a pointer to the last stack from the back

    if(!stacks.empty() && stack_pointer != NULL && stack_pointer->size() < stack_capacity) {  // step3 if the stacks list is not empty and there is space in the current stack
        stack_pointer->push(mp);  // step3.1 insert the new meal into the current stack
    } else if (stacks.size() < usable_stacks) {  // step4 else if there is space for a new stack
        stack<MealPortion*>* n_stack_p = new stack<MealPortion*>;  // step4.1 create a new stack with new stack<MealPortion*>
        n_stack_p->push(mp); // step4.2 add the meal portion to that stack
        stacks.push_back(n_stack_p); // step4.3 add the new stack to the stacks list
    } else {  // step5 else there is no more space for new stacks
//        cout << endl << "------- MEAL NOT ADDED =-" << endl;
        delete mp;
        return false;  // step5.1 return false
    }
//    cout << endl << "------- MEAL ADDED =-" << endl;

    return true;  // step6 return true if addition was successful
}
...
//    cout << endl << "------- START OF REMOVING MEAL =-" << endl;

    MealPortion m =  MealPortion("EMPTY", "N/A");  // step1 create a temporary MealPortion("EMPTY", "N/A") object called m

    if(stacks.empty()) {  // step2 if the stacks list is empty
//        cout << endl << "------- STACK WAS EMPTY, COULDN'T REMOVE =-" << endl;
        return m;  // step2.1 return corresponding m value
    }

    stack<MealPortion*>* stack_pointer = stacks.back();  // step3 grab a pointer to the stack at the back of the list

    m.name = stack_pointer->top()->name;  // step4 set m's value to match the value of top element of that stack
    m.expiry = stack_pointer->top()->expiry;
    delete stack_pointer->top();  // step5 free memory for the top element and pop it from the stack
    stack_pointer->pop();

    if (stack_pointer->empty()) {  // step6 if the current stack is empty
        delete stack_pointer;  // step6.1 free its memory and remove it from the list
        stacks.pop_back();
    }
//    cout << endl << "------- END OF REMOVING MEAL =-" << endl;
    return m;  // step7 return corresponding m value
}
...
FridgeOrganizer::MealPortion FridgeOrganizer::find_meal_portion_by_expiry() {
    ...
    MealPortion min = MealPortion("EMPTY", "N/A");  // step1 create a temporary MealPortion("EMPTY", "N/A") object called min
//    cout << endl << "------- FINDING MEAL BY EXPIRY =-" << endl;

    if(stacks.empty()) {  // step2 if the stacks list is empty
//        cout << endl << "------- STACK WAS EMPTY =-" << endl;
        return min;   // step2.1 return min value
    }

    for (list<stack<MealPortion*>*>::iterator i = stacks.begin(); i != stacks.end(); ++i) {  // step3 use list iterator to go through list elements
        stack<MealPortion*> cur_stack = **i;  // step3.1 create a local copy of the current stack; also, store its size as a variable
        int cur_stack_size = cur_stack.size();
        for (int cur_element_index = 0; cur_element_index < cur_stack_size; ++cur_element_index) {  // step3.2 iterate through the copy of the current stack using index value
            if(cur_stack.top()->expiry < min.expiry){  // step3.2.2 if the stack top's expiry date is less than min->expiry
                min = *cur_stack.top();  // step3.2.2.1 copy the stack top's value into min
            }
            cur_stack.pop();  // step3.2.3 pop the top element from the stack; do not free its memory
        }
    }
//    cout << endl << "------- FOUND MEAL BY EXPIRY-" << endl;
    return min;  // step4 return corresponding min value
}
